...
export const ROUTES = {
  ...
  inboxPending: 'direct_v2/pending_inbox/?use_unified_inbox=true<%= maxId ? ("&max_id=" + maxId) : "" %>',
  threads: 'direct_v2/threads/?user_ids=<% JSON.stringify(threads) %>',
  threadsShow: 'direct_v2/threads/<%= threadId %>/?use_unified_inbox=true<%= cursor ? ("&cursor=" + cursor) : "" %>',
  threadsSeen: 'direct_v2/threads/<%= threadId %>/items/<%= itemId %>/seen/?use_unified_inbox=true',
  threadsApprove: 'direct_v2/threads/<%= threadId %>/approve/',
  threadsHide: 'direct_v2/threads/<%= threadId %>/hide/?use_unified_inbox=true',
